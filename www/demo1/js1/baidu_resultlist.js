...
function showinfo(key,pageno){
		...
		if(xhr.readyState == 4){
			if(xhr.responseText == "数据为空！"){
				...
				for(var i = 0;i < len_arr;++i){
					...
					s +=			"<a href = " + str[i]. Shareurl + " class = 'a' style = 'font-weight:bold' target = '_bank'>" + str[i]. Title + "</a>";
					s +=			"<span style = 'display:inline-block;'>" + decodeURIComponent(key) + "</span><span style = 'display:inline-block;margin-right:20px;' class = 'pull-right'>" + str[i].Date + "</span>";
					s +=		"</div>";
					s +=	"</div>";
					s +=	"<div class = 'row re_menu' style = 'margin-bottom:0px;'onclick = 'modelbox(" + JSON.stringify(str[i]) + ")'>";
					...
					s +=	"</div>";
					s += 	"<div class = 'row'>";
					s +=		"<div class = 'col-md-12 col-sm-12 col-xs-12 col-lg-12 text-right'style = 'padding-right:40px;'>" + str[i].Size + "</div>";
					s +=	"</div>";
				}
	...
	xhr.open('get','?p=back&c=admin&act=baidu_keyword_details&name=' + key + '&pageno=' + pageno);
	xhr.send(null);	
}
//页面加载完毕显示分页页码
function page(opt){
